...
| Infobox is not a Statblock | Distinct blocks sharing one named primitive — the ordered labelled-row list. An Infobox is exactly a Statblock's *header* and nothing below it. | A Statblock additionally has named prose entries carrying a *kind* (the map's hardest constraint), values mutated during play at a write frequency an order of magnitude higher, and reuse/identity outside its note. Those are differences in kind, not field count. One node with a mode is precisely how the three shipped blocks drifted; nesting an Infobox as the header would drag floatability, a caption, and freeform keys into a statblock, all three wrong. |
