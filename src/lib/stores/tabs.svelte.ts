...
function createTabsStore() {
	...
	function persist() {
		untrack(() => {
			...
			const state: PersistedState = {
				left: { tabs: left.tabs.map(t => ({ type: t.type, id: t.id, title: t.title })), activeIndex: left.activeIndex },
				right: right ? { tabs: right.tabs.map(t => ({ type: t.type, id: t.id, title: t.title })), activeIndex: right.activeIndex } : null,
				focusedPane,
			};
			try { localStorage.setItem(storageKey(vault.path!), JSON.stringify(state)); } catch {}
		});
	}
	...
	function openTab(tab: Tab, targetPane?: 'left' | 'right') {
		...
		const leftIdx = left.tabs.findIndex(t => t.type === tab.type && t.id === tab.id);
		if (leftIdx !== -1) {
		...
		if (right) {
			const rightIdx = right.tabs.findIndex(t => t.type === tab.type && t.id === tab.id);
			if (rightIdx !== -1) {
	...
	function closeTab(pane: 'left' | 'right', index: number) {
		if (pane === 'right') {
			...
			if (tabs.length === 0) {
				...
				right = { tabs, activeIndex: Math.min(right.activeIndex, tabs.length - 1) };
			}
		} else {
			const tabs = [...left.tabs];
			tabs.splice(index, 1);
			left = { tabs, activeIndex: tabs.length === 0 ? 0 : Math.min(left.activeIndex, tabs.length - 1) };
		}
		persist();
	}
	...
	function closeTabByTypeAndId(type: TabType, id: number) {
		const leftIdx = left.tabs.findIndex(t => t.type === type && t.id === id);
		if (leftIdx !== -1) { closeTab('left', leftIdx); return; }
		if (right) {
			const rightIdx = right.tabs.findIndex(t => t.type === type && t.id === id);
	...
	return {
		...
		get leftActiveTab() { return left.tabs[left.activeIndex] ?? null; },
		get rightActiveTab() { return right ? (right.tabs[right.activeIndex] ?? null) : null; },
		get activeTab() {
			return focusedPane === 'left'
				? (left.tabs[left.activeIndex] ?? null)
				: (right ? (right.tabs[right.activeIndex] ?? null) : null);
		},
		openTab,
		openTabWithRename,
		closeTab,
