<!-- src/lib/components/panes/ScenePicker.svelte
     ...
     chip. Used by the PDF "+ Link Scene" flow. Existing Scenes only (this slice);
     the design's footer "＋ New scene" affordance is deferred. -->
<script lang="ts">
  import { Search, Music2 } from "@lucide/svelte";
  ...
  <div class="flex max-h-56 flex-col overflow-y-auto" role="listbox" aria-label="Scenes">
    {#if scenes.scenes.length === 0}
      <p class="px-2 py-2 font-sans text-xs text-muted-foreground/50">
        No scenes yet. Create one in Scenes first.
      </p>
