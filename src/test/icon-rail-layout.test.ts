	...
	beforeEach(async () => {
		vi.mocked(invoke).mockImplementation(async (cmd: string) => {
			...
			if (cmd === 'get_recent_vaults') return [
				{ name: 'MyVault', path: '/Users/test/MyVault', note_count: 5, scene_count: 2, map_count: 1, last_opened: '2025-01-01T00:00:00Z' },
				{ name: 'OtherVault', path: '/Users/test/OtherVault', note_count: 0, scene_count: 0, map_count: 0, last_opened: '2025-01-02T00:00:00Z' },
			];
			return null;
		});
...
describe('icon rail', () => {
	...
	it('Files button click expands the sidebar when it is closed', async () => {
		...
		const sidebarAfterCollapse = document.querySelector('[data-slot="sidebar"][data-state]');
		...
		const sidebarAfterExpand = document.querySelector('[data-slot="sidebar"][data-state]');
