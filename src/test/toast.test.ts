...
describe('toastError', () => {
	it('calls sonner.error with duration Infinity', () => {
		...
		expect(sonner.error).toHaveBeenCalledWith('Something broke', expect.objectContaining({ duration: Infinity }));
	});
});
...
describe('toastUndo', () => {
	...
	it('shows a toast with an Undo action', () => {
		...
		expect(sonner).toHaveBeenCalledWith('Note deleted', expect.objectContaining({
	...
	it('does NOT execute onConfirm when undo action is clicked', () => {
		...
		const options = vi.mocked(sonner).mock.calls[0][1] as unknown as { action: { onClick: (e: MouseEvent) => void } };
