...
describe('icon rail — density tokens', () => {
  it('nav icons use --icon-rail-icon size variable', () => {
    ...
    const allVariableSize = svgs.every((svg) =>
      svg.classList.contains('size-[var(--icon-rail-icon)]')
    );
...
describe('AppSearch — density tokens', () => {
  it('search bar uses --row-h height token', () => {
    render(AppShell);
    const searchBar = document.body.querySelector('[data-testid="app-search-bar"]')!;
    expect(searchBar).toBeTruthy();
    expect(searchBar.classList.contains('h-[var(--row-h)]')).toBe(true);
  });
  ...
  it('search bar uses --pad-x padding token', () => {
    ...
    const searchBar = document.body.querySelector('[data-testid="app-search-bar"]')!;
    expect(searchBar).toBeTruthy();
    expect(searchBar.classList.contains('px-[var(--pad-x)]')).toBe(true);
  });
});
...
describe('sidebar group labels — density tokens', () => {
  it('group labels use --row-h height token', () => {
    render(AppShell);
    const labels = document.body.querySelectorAll('[data-sidebar="group-label"]');
    expect(labels.length).toBeGreaterThan(0);
    for (const label of Array.from(labels)) {
      expect(label.classList.contains('h-[var(--row-h)]')).toBe(true);
    }
  });
